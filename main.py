...
from PyQt6.QtGui import QPixmap, QFont, QIcon, QActionGroup
...
class CigaretteBatteryWidget(QWidget):
    def __init__(self):
        ...
        # --- Early Init (fixes test_mode error) ---
    ...
    def update_battery(self):
        ...
        if os.path.exists(sprite_path):
            pixmap = QPixmap(sprite_path).scaled(
                128, 128, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.FastTransformation
            )
    ...
    def handle_glow_animation(self):
        if self.charging:
            if self.sprite_animation.state() == 0:
    ...
    def init_tray_icon(self):
        icon_path = os.path.join(self.asset_path, "cig_full.png")
        tray_icon = QIcon(icon_path) if os.path.exists(icon_path) else QIcon()

        self.tray = QSystemTrayIcon(tray_icon, self)
        self.tray.setToolTip("Cigarette Battery Widget")
        ...
        # Test mode toggle
        self.test_action = menu.addAction("Enable Test Mode")
        self.test_action.triggered.connect(self.toggle_test_mode)
